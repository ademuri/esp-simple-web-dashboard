...
class Dashboard {
  public:
    ...
    void Add(std::string name, std::function<std::string ()> callback, uint32_t refresh_millis);
    ...
    template <typename Type>
    void Add(std::string name, std::function<Type ()> callback, uint32_t refresh_millis);
...
template <typename Type>
void Dashboard::Add(std::string name, Type &value_ptr, uint32_t refresh_millis) {
  Add(name, [&value_ptr]() { 
...
template <typename Type>
void Dashboard::Add(std::string name, std::function<Type ()> callback, uint32_t refresh_millis) {
  Add(name, [callback]() { 
    std::ostringstream stream;
    stream << callback();
    return stream.str();
