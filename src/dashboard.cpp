...
void Dashboard::Add(std::string name, std::function<std::string ()> callback, uint32_t refresh_millis) {
  // Add element to template
  std::ostringstream list_stream;
  list_stream << "<div><span class='label'>" << name << "</span>: <span id='value-" << std::dec << next_id_ << "'></span><span class='activity' id='activity-" << next_id_ << "'></span></div>\n";
  list += list_stream.str().c_str();

  std::ostringstream script_stream;
  script_stream << "getValue('" << std::dec << next_id_ << "');\n";
  script_stream << "setInterval(getValue.bind(undefined, '" << std::dec << next_id_ << "'), " << refresh_millis << ");\n";
  script += script_stream.str().c_str();

  // Register endpoint for getting value
  String path = String("/get-") + String(next_id_); 
  server_->on(path.c_str(), HTTP_GET, [callback, this](AsyncWebServerRequest *request) {
    last_used_ = millis();
...
void Dashboard::Add(std::string name, bool &value_ptr, uint32_t refresh_millis) {
  Add(name, [&value_ptr]() { 
    return value_ptr ? "true" : "false";
  }, refresh_millis);
}

uint32_t Dashboard::last_used() {
  return last_used_;
}
